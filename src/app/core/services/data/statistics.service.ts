...
export class StatisticsService {
   ...
   */
  private static isMedicineStatistics(
    array: MedicineStatisticsDto[] | object
  ): array is MedicineStatisticsDto {
   ...
   */
  private static isUserStatistics(
    array: UserStatisticsDto[] | object
  ): array is UserStatisticsDto[] {
  ...
  getMedicinesStatistics(): Observable<MedicineStatisticsModel[]> {
    return this.database
      .list("/statistics/medicines/")
      .valueChanges()
      .pipe(
        map(records => {
          const dtoArr: MedicineStatisticsDto[] =
            ProjectFunctions.mapObjectToArray(records);
          return StatisticsService.mapDtoArrayToModelArray(
            dtoArr
          ) as MedicineStatisticsModel[];
        })
      );
  }
  ...
  getStaffStatistics(): Observable<PeopleStatisticsModel[]> {
    return this.database
      .list("/statistics/staff/")
      .valueChanges()
      .pipe(
        map(records => {
          const dtoArr: PeopleStatisticsDto[] =
            ProjectFunctions.mapObjectToArray(records);
          return StatisticsService.mapDtoArrayToModelArray(
            dtoArr
          ) as PeopleStatisticsModel[];
        })
      );
  }
  ...
  getUsersStatistics(): Observable<UserStatisticsModel[]> {
    return this.database
      .list("/statistics/users/")
      .valueChanges()
      .pipe(
        map(records => {
          const dtoArr: UserStatisticsDto[] =
            ProjectFunctions.mapObjectToArray(records);
          return StatisticsService.mapDtoArrayToModelArray(
            dtoArr
          ) as UserStatisticsModel[];
        })
      );
  }
  ...
  writeSuccessfulTransaction(transaction: BookingModel): void {
    ...
    for (const place of statisticsPlaces) {
      if (place === "users/") {
        this.processTransactionRequestOnPath(
          basePath + place,
          transaction,
          UserStatus.Success
        );
   ...
   */
  private writeMedicines(
    medicinesDto: MedicineStatisticsDto[],
    ...
    for (const medicine of medicines) {
      const appropriateRecord = medicinesDto.find(
        x => x.term === medicine.name
      );
      if (appropriateRecord) {
        this.database
          .object(`/statistics/medicines/${appropriateRecord.key}/purchased`)
          .set(appropriateRecord.purchased + medicine.amount);
      } else {
   ...
   */
  private writeStaff(
    staffDto: PeopleStatisticsDto | undefined,
    ...
    const itemsCount = transaction.medicines.reduce(
    ...
    if (staffDto) {
      this.database
        .object(`/statistics/staff/${staffDto.key}/purchasedItems`)
        .set(staffDto.purchasedItems + itemsCount);
    } else {
   ...
   */
  private writeSuccessfulUser(
    userDto: UserStatisticsDto | undefined,
    transaction: BookingModel
  ): void {
    const itemsCount = transaction.medicines.reduce(
      (accumulated, { amount }) => accumulated + amount,
    ...
    if (userDto) {
      this.database
        .object(`/statistics/users/${userDto.key}/purchasedItems`)
        .set(userDto.purchasedItems + itemsCount);
    } else {
  ...
  private writeCancelledUser(
    ...
    if (userDto) {
      this.database
        .object(`/statistics/users/${userDto.key}/cancelledBookings`)
        .set(userDto.cancelledBookings + 1);
    } else {
      this.database.list(`/statistics/users/`).push(
        Object.assign(
          {},
  ...
  private writeFailedUser(
    ...
    if (userDto) {
      this.database
        .object(`/statistics/users/${userDto.key}/failures`)
        .set(userDto.failures + 1);
    } else {
      this.database.list(`/statistics/users/`).push(
        Object.assign(
          {},
  ...
  private processTransactionRequestOnPath(
    path: string,
    transaction: BookingModel,
        ...
        tap(
          (
            ...
            if (path.includes("medicines")) {
              this.writeMedicines(
                records as MedicineStatisticsDto[],
                transaction.medicines
              );
            } else if (path.includes("users")) {
              const userDto = (records as UserStatisticsDto[]).find(
                x => x.email === transaction.email
              );
              switch (userStatus) {
              ...
              const staffDto = (records as PeopleStatisticsDto[]).find(
                x => x.email === staffEmail
              );
