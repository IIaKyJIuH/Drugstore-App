...
export class BookingsComponent {
  ...
  constructor(
    ...
    this.allBookings$ =
      userRole === "STAFF" ? this.getAllBookings() : this.getMyBookings();
  }
  ...
  getAllBookings(): Observable<BookingModel[]> {
    return this.bookingsService.getAllBookings().pipe(
      map((bookingsArr: BookingModel[]) => {
        const savedSpike: BookingModel[] = localStorage.getItem("savedSpike")
          ? JSON.parse(localStorage.getItem("savedSpike"))
          : [];
  ...
  private removeFromLocalStorage(transaction: BookingModel): void {
    const savedSpike: BookingModel[] = localStorage.getItem("savedSpike")
      ? JSON.parse(localStorage.getItem("savedSpike"))
      : [];
    const localRecordIndex = savedSpike.findIndex(
      x => x.key === transaction.key
    );
